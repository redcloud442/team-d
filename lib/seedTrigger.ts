...
async function main() {
  ...
  await sql`
    create or replace function user_schema.handle_new_user()
    returns trigger as $$
    begin
  ...
  await sql`
    create or replace trigger on_auth_user_created
      ...
      for each row execute procedure user_schema.handle_new_user();
  `;

  // Create function to handle user deletions
  await sql`
    create or replace function user_schema.handle_user_delete()
    returns trigger as $$
    begin
